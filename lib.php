...
/**
 * Library of interface functions and constants for module peer
 *
 * All the core Moodle functions, neeeded to allow the module to work
 * integrated in Moodle should be placed here.
 *
 * All the peer specific functions, needed to implement all the module
 ...
 * of the new instance.
 *
 * @param stdClass $peer Submitted data from the form in mod_form.php
 * @param mod_peer_mod_form $mform The form instance itself (if needed)
 * @return int The id of the newly inserted peer record
 */
function peer_add_instance(stdClass $peer, mod_peer_mod_form $mform = null) {
    global $DB;
...
/**
 * Updates an instance of the peer in the database
 *
 * Given an object containing all the necessary data,
 * (defined by the form in mod_form.php) this function
 ...
 */
function peer_update_instance(stdClass $peer, mod_peer_mod_form $mform = null) {
    global $DB;
    ...
    $result = $DB->update_record('peer', $peer);
...
/**
 * Removes an instance of the peer from the database
 *
 ...
 *
 * @param int $peerid ID of an instance of this module
 * @param int $scaleid ID of the scale
 * @return bool true if the scale is used by the given peer instance
 */
function peer_scale_used($peerid, $scaleid) {
    global $DB;

    if ($scaleid and $DB->record_exists('peer', array('id' => $peerid, 'grade' => -$scaleid))) {
        return true;
    } else {
        return false;
    }
...
function peer_scale_used_anywhere($scaleid) {
    ...
    if ($scaleid and $DB->record_exists('peer', array('grade' => -$scaleid))) {
        return true;
    } else {
        return false;
    }
...
/**
 * Creates or updates grade item for the given peer instance
 *
 ...
 * @return void
 */
function peer_grade_item_update(stdClass $peer, $reset=false) {
    global $CFG;
    require_once($CFG->libdir.'/gradelib.php');

    $item = array();
    $item['itemname'] = clean_param($peer->name, PARAM_NOTAGS);
    $item['gradetype'] = GRADE_TYPE_VALUE;

    if ($peer->grade > 0) {
        $item['gradetype'] = GRADE_TYPE_VALUE;
        $item['grademax']  = $peer->grade;
        $item['grademin']  = 0;
    } else if ($peer->grade < 0) {
        $item['gradetype'] = GRADE_TYPE_SCALE;
        $item['scaleid']   = -$peer->grade;
    } else {
 ...
 */
function peer_update_grades(stdClass $peer, $userid = 0) {
    global $CFG, $DB;
    require_once($CFG->libdir.'/gradelib.php');

    // Populate array of grade objects indexed by userid.
    $grades = array('userid'=>$userid,'rawgrade'=>0,'finalgrade'=>0);
    
   

    grade_update('mod/peer', $peer->course, 'mod', 'peer', $peer->id, 0, $grades);
}
...
/**
 * Extends the global navigation tree by adding peer nodes if there is a relevant content
 *
 * This can be called by an AJAX request so do not rely on $PAGE as it might not be set up properly.
 *
 * @param navigation_node $navref An object representing the navigation tree node of the peer module instance
 * @param stdClass $course current course record
 * @param stdClass $module current peer instance record
 * @param cm_info $cm course module information
 */
function peer_extend_navigation(navigation_node $navref, stdClass $course, stdClass $module, cm_info $cm) {
    // TODO Delete this function and its docblock, or implement it.
}

/**
 * Extends the settings navigation with the peer settings
 *
 * This function is called when the context for the page is a peer module. This is not called by AJAX
 * so it is safe to rely on the $PAGE.
 *
 * @param settings_navigation $settingsnav complete settings navigation tree
 * @param navigation_node $peernode peer administration node
 */
function peer_extend_settings_navigation(settings_navigation $settingsnav, navigation_node $peernode=null) {
    // TODO Delete this function and its docblock, or implement it.
}
